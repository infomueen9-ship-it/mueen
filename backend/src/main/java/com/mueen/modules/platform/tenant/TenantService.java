...
jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS " + p + "classrooms (id BIGSERIAL PRIMARY KEY, name VARCHAR(100) NOT NULL)");
        ...
        jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS " + p + "classroom_subjects (id BIGSERIAL PRIMARY KEY, classroom_id BIGINT NOT NULL REFERENCES " + p + "classrooms(id) ON DELETE CASCADE, name VARCHAR(100) NOT NULL, teacher_id BIGINT REFERENCES " + p + "teachers(id), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(classroom_id, name))");
